...
for rally_id in df["rally_id"].unique():
    ...
    # Target: opponent’s return shot after last Djokovic hit
    last_frame = dj_hits.iloc[min(2, len(dj_hits)-1)]["frame_id"]
    opp_return = rally_bounces[
        (rally_bounces["frame_id"] > last_frame) &
        (rally_bounces["player"] == "Opponent (Op)")
    ].sort_values("frame_id")

    if not opp_return.empty:
        row["target_x"] = opp_return.iloc[0]["x_m"]
        row["target_y"] = opp_return.iloc[0]["y_m"]
    else:
        row["target_x"] = np.nan
        row["target_y"] = np.nan
...
features_df.to_excel("excel/djokovic_features_3shots_partial1.xlsx", index=False)

print(f"\n✅ Final output: Extracted {len(features_df)} rows → excel/djokovic_features_3shots_partial1.xlsx")
